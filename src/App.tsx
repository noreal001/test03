...
import { Box, List, ListItemButton, ListItemText, Typography, Paper, Fade, Divider, IconButton, TextField, InputAdornment, Chip, Stack, Avatar, useMediaQuery, useTheme, Button } from '@mui/material';
...
const App: React.FC = () => {
  ...
  const [profileTab, setProfileTab] = useState<'fio' | 'address' | 'orders'>('fio');
  ...
  const [phone, setPhone] = useState('');     // New state for phone input
  ...
  const handlePaymentComplete = () => {
    ...
    setRightPanelContent('profile'); // Switch to profile panel
    setProfileTab('orders'); // Open orders tab
  ...
  return (
    console.log('Current isProfileFullScreen state (before render):', isProfileFullScreen),
    <Box sx={{ minHeight: '100vh', width: '100vw', bgcolor: 'background.paper', color: 'text.primary', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ position: 'fixed', top: 0, right: 0, p: 2, zIndex: 2000, display: 'flex', alignItems: 'center', gap: 1 }}>
        <IconButton 
          ...
          onClick={() => {
            ...
            setRightPanelContent(null); // Close any open right panel (cart/profile sidebar)
      ...
      <Box sx={{ flex: 1, display: 'flex', flexDirection: isMobile ? 'column' : 'row', p: 0, pt: '64px', overflowX: 'hidden', alignItems: 'stretch' }}>
      ...
      {/* Меню брендов слева */}
        {brandsMenuOpen && (
          <Paper elevation={3} sx={{ width: 340, minWidth: 340, maxWidth: 340, bgcolor: '#000', p: 2, pr: 2, pb: 0, mr: 0, mb: isMobile ? 2 : 0, flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', position: 'relative', height: '100%', justifyContent: 'flex-start', borderTopRightRadius: 0, borderBottomRightRadius: 0, boxShadow: 'none', boxSizing: 'border-box', bottom: 0 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', width: '100%', mb: 1 }}>
              ...
              <IconButton 
                sx={{ color: 'text.primary', mt: '-8px' }} /* Поднимаем иконку */
                onClick={() => {
        ...
        {!brandsMenuOpen && (
          <Box sx={{ width: 66, minWidth: 66, maxWidth: 66, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'flex-start', height: '100%', bgcolor: 'background.paper', opacity: 0.96, boxShadow: 3, borderRight: '2px solid #222', pt: 2, position: 'relative' }}>
            ...
            <IconButton 
              sx={{ ml: 1, color: 'text.primary', mt: '-8px' }} /* Поднимаем иконку */
              onClick={() => {
                setIsProfileFullScreen(true); // Всегда открываем профиль
                setRightPanelContent(null); 
                setIsCartFullScreen(false); 
        ...
        {/* Центральная панель (ароматы/поиск/полноэкранный профиль/полноэкранная корзина) */}
          ...
          >
            ...
            {isProfileFullScreen ? (
              <Paper elevation={6} sx={{ flex: 1, bgcolor: '#000', color: '#fff', p: 3, display: 'flex', flexDirection: 'column', height: '100%', borderRadius: 0, boxShadow: 'none' }}>
                  ...
                  <Box sx={{ flex: 1, minWidth: 220, p: 3, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                    {profileTab === 'fio' && (
                      <>
                        ...
                        <TextField fullWidth size="small" value={user.name} onChange={(e) => setUser(prev => ({...prev, name: e.target.value}))} sx={{ input: { color: '#fff' } }} />
                      </>
                    )}
                    {profileTab === 'address' && (
                      <>
                        ...
                        {user.orders.length === 0 ? (
                          ...
                          <List>
                            {user.orders.map((order, index) => (
                              ...
                              }} sx={{ mb: 1, bgcolor: '#333', borderRadius: 1 }}>
                                <ListItemText
              ...
              <Paper elevation={6} sx={{ flex: 1, bgcolor: '#000', color: '#fff', p: 3, display: 'flex', flexDirection: 'column', height: '100%', borderRadius: 0, boxShadow: 'none' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                  ...
                  <IconButton onClick={() => setIsCartFullScreen(false)}><CloseIcon /></IconButton>
                </Box>
                {cart.length === 0 ? (
                  ...
                  <List sx={{ flex: 1, overflowY: 'auto' }}>
                    {cart.map((item, idx) => (
                      <ListItemButton key={idx} sx={{ mb: 1, bgcolor: '#333', borderRadius: 1 }}
                        onClick={() => {
              ...
              <Fade in={selectedIndex !== null} timeout={400} unmountOnExit>
                <Paper elevation={4} sx={{ width: '100%', p: 3, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 2, position: 'relative', flex: 1, minHeight: 0, overflowY: 'auto', boxSizing: 'border-sizing' }}>
        ...
        {/* Единая правая панель (Корзина) */}
        {!isProfileFullScreen && !isCartFullScreen && rightPanelContent === 'cart' && (
          <Paper elevation={6} sx={{ width: 340, minWidth: 340, maxWidth: 340, height: '100%', zIndex: 1999, p: 3, bgcolor: '#000', color: '#fff', display: 'flex', flexDirection: 'column', flexShrink: 0, transition: 'width 0.3s' }}>
              <>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
            <Typography variant="h6" sx={{ flex: 1, fontWeight: 700 }}>Корзина</Typography>
